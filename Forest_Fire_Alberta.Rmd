...
output:
  ...
  html_document:
    df_print: paged
---
# DATA 602 Group Project - Forest Fires in Alberta
...
library(leaflet)
```
In order to perform this analysis, we used data from the Historical Wildfire Dataset: 2006 to 2023[^3].  This dataset was provide by the Alberta government and include many continuous variables such as fire size, wind speed, temperature, and response times.
...
#head(data) #masking output here to avoid clutter, as we have seen this before, want to leave it in to show that team member used it for visual inspection
...
### EDA
```{r}
#head(data1) #masked this output, as we have it before.  Want to leave it in, to show that this member started by looking at the head()
```
...
2. Natural Resources Canada. (n.d.). National fire database – Agency fire data. Canadian Wildland Fire Information System. Retrieved October 15, 2024, from https://cwfis.cfs.nrcan.gc.ca/ha/nfdb

3. Government of Alberta. (n.d.). Wildfire data [Data set]. Open Alberta. https://open.alberta.ca/opendata/wildfire-data#summary
